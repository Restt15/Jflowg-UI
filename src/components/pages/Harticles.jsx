...
const Harticles = ({ addToCart }) => {
  ...
  <section className="px-4 sm:px-6 lg:px-8 py-8 flex flex-col sm:flex-row items-start justify-between">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-16">
      {foods.map((item) => (
        <div key={item.id} className="block w-full sm:w-auto">
          <Card className="rounded-lg overflow-hidden shadow-md border-gray-500 border-opacity-20 hover:border-opacity-50 transition duration-300 ease-in-out transform hover:scale-105">
            <a href={`/Harticles/Details/${item.id}`}>
              <Card className="relative overflow-hidden group border-8 border-y-indigo-900">
                <img
                  src={item.Image}
                  alt={item.title}
                  className="w-full h-64 object-cover object-center neon-border transform transition-transform group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-indigo-900  via-transparent to-indigo-900 opacity-0 group-hover:opacity-50"></div>
              ...
              <CardBody className="p-4">
                ...
                <div className="flex items-center justify-between">
                  <Typography className="text-blue-900 font-semibold text-lg">
    ...
    {/* Mensaje de Alerta */}
    {alertMessage && (
  <div className="fixed top-20 lg:top-16   md:top-4 right-6 p-4 bg-white text-black border border-gray-300 rounded-lg shadow-md transition-transform duration-500 transform translate-y-0 max-w-sm">
